...
#Loading the dataset from Snowflake to Vs code editer
df1 =session.table('HACKATHON.DEATHES.BLACKSPOT_19').to_pandas()
df2 =session.table('HACKATHON.DEATHES.ROAD_DEATHES').to_pandas()
df3 =session.table('HACKATHON.DEATHES.LICENSE').to_pandas()
df4 =session.table('HACKATHON.DEATHES.ROAD_DEATHES').to_pandas()
df5 =session.table('HACKATHON.DEATHES.PROFESSIONAL_19').to_pandas()
df6 =session.table('ACCIDENTHACKATHON.BLACKSPOT_2019.BLACKSPOT2019').to_pandas()
df7 =session.table('HACKATHON.TOTALROADVSPOPULATION18_19.TOTALROADVSPOPULATION16_17').to_pandas()

# Checking if the states have the same value present or not 
...
df5['Professional - Authorised to drive public Service Vehicles - Total'] = df5['Professional - Authorised to drive public Service Vehicles - Total'].fillna(0)
df5['Non professional - Total'] = df5['Non professional - Total'].fillna(0)

df5['Non_Professional vs Professional'] = df5['Non professional - Total'] // df5['Professional - Authorised to drive public Service Vehicles - Total']
#df10['Non_Professiinal vs Professional'].fillna(0)
df5['Non_Professional vs Professional Authorised Driver'] = df5['Non_Professional vs Professional'].fillna(0).replace([np.inf, -np.inf], 0)

if selected_option:
    st.write(f'You selected: {selected_option}')

    # # text = input(" Enter the State : ")
    selected_row = df5[df5["STATES / Uts"]==selected_option]
...
def plot_driver_distribution(text):
    row = df5[df5['STATES / Uts'] == text].iloc[0]
